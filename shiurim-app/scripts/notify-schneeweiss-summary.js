...
// migrate-schneeweiss.js run (see .github/workflows/schneeweiss-weekly-sync.yml).
//
...
// migration script crashed outright. A quiet week (nothing new on YUTorah)
// sends nothing.
// =============================================================================
const fs = require('fs');
...
async function main() {
  ...
  if (MIGRATE_OUTCOME !== 'success' || !summaryMatch) {
    await send(
      ...
      `<h2>Weekly Rabbi Schneeweiss YUTorah sync failed</h2>
  ...
  const html = `
    <h2>Weekly Rabbi Schneeweiss YUTorah sync</h2>
